...
from cryozeta.data import ccd
from cryozeta.data.ccd import get_ccd_ref_info
...
from cryozeta.data.utils import get_starts_by
...
class AddAtomArrayAnnot:
    ...
    def find_equiv_mol_and_assign_ids(
        ...
        assert len(np.unique(atom_array.mol_id)) == len(mol_indices), (
        ...
        for _mol_id, atom_indices in enumerate(mol_indices):
        ...
        for start, stop in itertools.pairwise(mol_starts):
        ...
        for i in range(num_mols):
            for j in range(i + 1, num_mols):
                ...
                for key in ["res_name", "atom_name", "mol_atom_index"]:
                    ...
                    assert np.all(annot[start_i:stop_i] == annot[start_j:stop_j]), (
