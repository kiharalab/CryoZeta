...
from cryozeta.data.data_transforms_em import (
    get_pointpair,
    get_pointresiduepair,
    make_seq_mask,
    parse_mrc,
)
from cryozeta.data.featurizer import Featurizer
from cryozeta.data.json_parser import add_entity_atom_array, remove_leaving_atoms
from cryozeta.data.parser import AddAtomArrayAnnot
from cryozeta.data.tokenizer import AtomArrayTokenizer, TokenArray


class SampleDictToFeatures:
    def __init__(self, single_sample_dict, em_file_dir):
    ...
    def get_feature_dict(self) -> tuple[dict[str, torch.Tensor], AtomArray, TokenArray]:
        ...
        feature_dict["pdb_id"] = self.single_sample_dict["name"]
        map_path = self.single_sample_dict.get("map_path", None)
        feature_dict["map_path"] = map_path
        feature_dict["resolution"] = self.single_sample_dict.get("resolution", None)
        feature_dict["contour_level"] = self.single_sample_dict.get(
            "contour_level", None
        )
        ...
        feature_dict = get_pointpair(feature_dict, token_array, atom_array)
        feature_dict = get_pointresiduepair(feature_dict)
