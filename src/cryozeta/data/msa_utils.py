...
from cryozeta.data.constants import (
    PRO_STD_RESIDUES,
...
from cryozeta.openfold_local.data.msa_identifiers import (
    Identifiers,
    _extract_sequence_identifier,
    _parse_sequence_identifier,
)
from cryozeta.openfold_local.data.msa_pairing import (
    CHAIN_FEATURES,
    MSA_FEATURES,
    MSA_GAP_IDX,
    MSA_PAD_VALUES,
...
from cryozeta.openfold_local.np import residue_constants
...
try:
    from cryozeta.openfold_local.data.tools import jackhmmer
except ImportError:
    print(
...
def calc_stockholm_RNA_msa(
    name_to_sequence: OrderedDict, query: str | None
    ...
    if query is not None:
        ...
        if len(name_to_sequence.keys()) == 0:
            ...
            query = align_query_to_sto(query, next(iter(name_to_sequence.values())))
...
def parse_msa_data(
    ...
    """
    if msa_entity_type in ["prot", "rna"]:
...
def parse_rna_msa_data(
    ...
    """
    msa_data = {}
    for path, _seq_limit in zip(raw_msa_paths, seq_limits):
...
def load_and_process_msa(
    ...
    raw_msa_paths: Sequence[str],
    seq_limits: Sequence[int],
    identifier_func: Callable | None = lambda x: Identifiers(),
