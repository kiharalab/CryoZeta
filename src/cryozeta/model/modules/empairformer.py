...
from cryozeta.model.modules.em_module import (
    InterMultiplicativeUpdateIncoming,
    InterMultiplicativeUpdateOutcomming,
    PairUpdate,
    PointColumnAttentionWithPairBias,
    PointResidueTransition,
    ResidueRowAttentionWithPairBias,
)
from cryozeta.model.modules.primitives import LinearNoBias, Transition
from cryozeta.model.modules.transformer import AttentionPairBias
from cryozeta.model.utils import sample_msa_feature_dict_random_without_replacement
from cryozeta.openfold_local.model.dropout import DropoutRowwise
from cryozeta.openfold_local.model.outer_product_mean import (
...
from cryozeta.openfold_local.utils.checkpointing import checkpoint_blocks
...
class EMPairformerBlock(nn.Module):
    def __init__(
        ...
        self.pair_update_layer = PairUpdate(
            c_z=c_z,
        ...
        self.inter_update_outgoing = InterMultiplicativeUpdateOutcomming(
            c_z,
            c_p,
            c_hidden_mul,
            c_pz,
        ...
        self.inter_update_incomming = InterMultiplicativeUpdateIncoming(
            c_z,
            c_p,
            c_hidden_mul,
            c_pz,
        ...
        self.residue_row_attn = ResidueRowAttentionWithPairBias(
            c_pz=c_pz,
            c_z=c_z,
            c_hidden=c_hidden_msa_att,  # same output dimension as MSA row attention
            no_heads=no_heads_msa,
            inf=inf,
        )

        self.point_column_attn = PointColumnAttentionWithPairBias(
            c_pz=c_pz,
            c_p=c_p,
        ...
        self.point_resi_transition = PointResidueTransition(
            c_pz,
            transition_n,
        )

        self.outer_row_product_mean = OuterProductMean(
            c_pz,
            c_z,
            c_hidden_mul,
        )
    ...
    def forward(
        ...
        z = add(
            z,
            self.outer_row_product_mean(
                pz, mask=None, chunk_size=chunk_size, inplace_safe=inplace_safe
            ),
            inplace=inplace_safe,
        )

        ### Residue pair update ends

        ### Point pair update starts, added by Yuanyuan
        pz = pz.transpose(-2, -3)
        p = add(
            p,
            self.outter_column_product_mean(
                pz, mask=None, chunk_size=chunk_size, inplace_safe=inplace_safe
            ),
            inplace=inplace_safe,
        )
        pz = pz.transpose(-2, -3)
        ### Point pair update ends
...
class EMPairformerStack(nn.Module):
    ...
    def forward(
        ...
        s: torch.Tensor,
        z: torch.Tensor,
        pz: torch.Tensor,  # Point-residue representation
        p: torch.Tensor,  # Point pair representation
        pair_mask: torch.Tensor,
        use_memory_efficient_kernel: bool = False,
        use_deepspeed_evo_attention: bool = False,
        use_cuequivariance_attention: bool = False,
