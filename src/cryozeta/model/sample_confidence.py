...
from cryozeta.metrics.clash import Clash
from cryozeta.utils.distributed import traverse_and_aggregate
...
def calculate_ptm(
    ...
    max_bin: float,
    no_bins: int,
    token_mask: torch.BoolTensor | None = None,
) -> torch.Tensor:
    """Compute pTM score
...
def calculate_chain_based_ptm(
    ...
    for aid, _asym_mask in asym_id_to_asym_mask.items():
        pairs = [
            (i, j)
