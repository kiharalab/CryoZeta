...
def identify_mol_type(
    ...
    assert ref_space_uid.shape == atom_sums.shape, (
...
def get_spatial_crop_index(
    ...
    assert token_distance_mask[reference_token_idx].bool().any(), (
    ...
    assert selected_token_indices.shape[0] <= crop_size, (
...
class CropData:
    ...
    def crop_by_indices(
        ...
        msa_features: dict[str, np.ndarray] | None = None,
        template_features: dict[str, np.ndarray] | None = None,
    ...
    def get_crop_indices(self, crop_method: str | None = None) -> torch.Tensor:
        """
        ...
        """
        tokens, chain_id, token_dist_mask_1d, token_indices_in_ref, _is_ligand = (
            self.extract_info()
        )

        assert crop_method in self.cand_crop_methods, (
